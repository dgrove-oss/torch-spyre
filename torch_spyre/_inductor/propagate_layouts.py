...
def _multi_arg_pointwise_layouts(
    ...
    # A coarse-tiled multi-arg pointwise op with a broadcast operand (an input
    # whose host shape differs from the output, e.g.
    # `output / denominator.unsqueeze(-1)`) can spawn a degenerate sparse/bloated
    # output candidate whose device footprint exceeds the host element count
    # (from the sparse-stick sentinel path). When the op reads a coarse-tile full
    # accumulator whose own layout is that same degenerate shape, the beam commits
    # the bloated candidate; the operands then restickify into a dimension-
    # collapsed layout and the cross-tile read is misaligned (silent wrong result
    # -- flash-attention's out-of-tiling-scope `output / denominator` divide). If
    # an exact-footprint (dense) candidate exists, drop the bloated ones so the
    # dense layout is committed.
    #
    # Gated on loop_info (coarse-tile context): a standalone broadcast op outside
    # any tiling (e.g. a plain unsqueeze+broadcast add) legitimately needs its
    # larger-footprint candidate and must not be pruned. Reduction-accumulate
    # combines are same-shape (no broadcast operand) and are excluded too.
    if getattr(op, "loop_info", None) is not None and any(
        list(arg.layout.size) != list(output.size) for arg in args
    ):
        host_elems = (
