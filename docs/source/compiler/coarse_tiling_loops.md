...
appropriate for the counted, coarse-tiling use case. (`for_each_tile`'s own
lowering does route through `ir.WhileLoop` as a transient tracing artifact —
`torch._higher_order_ops.scan.scan` traces to one — but only because
`splice_while_loops` proves the trip count statically and splices the loop
body's ops directly into `graph.operations` before scheduling; the
`ir.WhileLoop` node itself never survives to become the final counted-loop
representation. See [What `_stamp_direct_loop_info`
stamps](#what-_stamp_direct_loop_info-stamps) above.)
